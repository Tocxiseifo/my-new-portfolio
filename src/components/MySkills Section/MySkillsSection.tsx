...
export default function MySkillsSection() {
    return(
        <motion.section initial={{opacity:0 , x:-100}} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: 0.1 }} transition={{duration:1 , ease:'easeOut' ,delay:0.3}} id="skills" className="h-auto lg:h-650  xl:max-h-340   w-full flex justify-center items-center flex-wrap gap-8 sm:gap-12 lg:gap-30 px-4 sm:px-6 lg:px-0 py-10 sm:py-14 lg:py-0 overflow-visible">
            {skillsData.map((skill,index) => (
                <motion.div initial={{opacity:0 , x:0}} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: 0.1 }} transition={{duration:1 , ease:'easeOut' ,delay:index * 0.6}} key={skill.id} className={skill.style}>
